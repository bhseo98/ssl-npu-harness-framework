...
| **2** | **문제 상황 인식** — Torch-MLIR 은 PyTorch 만 받음, HF transformers 는 PyTorch only 가 아님 → 자체 PyTorch-only LLM 모델링 필요 | [`REPORT-2026-05-20-phase2-mlir.md`](REPORT-2026-05-20-phase2-mlir.md) §1-2, [`agents/model-zoo.md`](agents/model-zoo.md) §"AMD-SHARK-AI" | ✅ |
| **3** | **AMD-SHARK-AI 분석** — `nod-ai/amd-shark-ai/amdsharktank` 가 Torch-MLIR lowering 성공 사례, On-Device 포팅 시 PagedAttention 제거 등 분석 | [`REPORT-2026-05-26-amd-shark-integration.md`](REPORT-2026-05-26-amd-shark-integration.md), [`docs/SHARK_AI_ANALYSIS.md`](docs/SHARK_AI_ANALYSIS.md) | ✅ |
...
| **5** | **PyTorch 기반 LLM E2E 모델링** — `meta-llama/Llama-3.2-1B-Instruct` 를 PyTorch 로 작성, MLIR 결과 IR dumping 분석 | [`src/torch_mlir_zoo/models/llama_on_device.py`](src/torch_mlir_zoo/models/llama_on_device.py) (171 LoC) + [`scripts/run_llama_export.py`](scripts/run_llama_export.py) + [`configs/zoo/llama_on_device_iree_turbine.yaml`](configs/zoo/llama_on_device_iree_turbine.yaml) + `.vmfb` 6.0 GB · [`REPORT-2026-05-27-llama-step5-complete.md`](REPORT-2026-05-27-llama-step5-complete.md) | ✅ |
...
|:---:|---|---|:---:|
| **1** | Naive Application Code (Python + PyTorch) on GPU Server — STT → LLM → TTS 한 turn | [`voice-app`](https://github.com/bhseo98/npu-harness-framework/tree/voice-app) | ✅ ([`docs/reports/2026-05-19-results.md`](docs/reports/2026-05-19-results.md)) |
| **2** | PyTorch Model to MLIR FRAMEWORK for target application — LLM/STT/TTS 의 *세 모델 모두* lowering | [`torch-mlir-zoo`](https://github.com/bhseo98/npu-harness-framework/tree/torch-mlir-zoo) | ⚠ 부분 |
...
|---|---|---|:---:|
| LLM | `Qwen/Qwen2.5-0.5B-Instruct` ⚠ vs lowering 대상 `Llama-3.2-1B` | ✅ Llama Step 5 완료. Qwen lowering 별도 추가 가능 ([`agents/open-questions.md`](agents/open-questions.md) Q1) | ⚠ |
| STT | `faster-whisper small` | ✗ — Whisper PyTorch-only 재구현 미진 (Gap C-1) | ✗ |
| TTS | `espnet_kss` (JETS) | ✗ — TTS lowering 후보 ([`agents/open-questions.md`](agents/open-questions.md) Q2) 미정 | ✗ |
...
→ 본 repo 의 산출물 `.vmfb` 가 점선 밖 IREE Runtime/HAL 까지는 *형식적* 호환 (CPU target). 실제 target NPU HW 까지의 통합은 외부 (compiler stack) 의 VP / driver / SoC 모델 수령 후 ([`agents/vp.md`](agents/vp.md), Deliverable 3 phase).
...
| **Medium** | Qwen-0.5B lowering 추가 ([`open-questions.md`](agents/open-questions.md) Q1 결정 후) | Deliverable 2 의 "target application" 정합 — voice-app default LLM 과 동일 모델 | 1 |
| **Low** | `--iree-llvmcpu-target-cpu=host` 재컴파일 + 성능 비교 | iree-compile generic CPU warning 해소 | 0.5 |
| **Low** | TTS lowering (Gap C-2) | [`open-questions.md`](agents/open-questions.md) Q2 설계 검토 답변 후 | 2-3 |
| **Blocked** | **Deliverable 3 (VP 진입)** | 프로젝트 리드 VP / driver / SoC 모델 수령 대기 ([`agents/vp.md`](agents/vp.md)) | — |
...
ls REPORT-2026-05-20-phase2-mlir.md REPORT-2026-05-26-amd-shark-integration.md REPORT-2026-05-27-llama-step5-complete.md

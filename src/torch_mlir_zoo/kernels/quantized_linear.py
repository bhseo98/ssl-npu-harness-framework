...
def quantize_linears_(module: nn.Module, block_size: int = 32) -> nn.Module:
    ...
    for name, child in list(module.named_children()):
        if isinstance(child, nn.Linear) and child.in_features % block_size == 0:
            setattr(module, name, BlockScaledQ8Linear.from_linear(child, block_size))
        else:
            quantize_linears_(child, block_size)
    return module

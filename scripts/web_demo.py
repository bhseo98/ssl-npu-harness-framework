...
MIN_INPUT_RMS = 0.001
DEMO_BUILD = "pcm16-worklet-default-rate-2026-05-18a"
...
<head>
  ...
  <title>NPU Harness Framework Voice</title>
  <style>
    :root {
      color-scheme: light;
      --ink: #151719;
    ...
    * { box-sizing: border-box; }
    ...
    #stopBtn:disabled { background: #a6b4bd; }
    .warn {
      margin-top: 16px;
      padding: 12px 14px;
    ...
    @media (max-width: 720px) {
      header, .dialogue, .controls, .device { grid-template-columns: 1fr; }
...
<body>
  <main>
    <header>
      <h1>NPU Harness Framework Voice</h1>
    ...
    <div class="device">
      <select id="micSelect" aria-label="Microphone"></select>
      <button id="refreshBtn">Refresh</button>
      <button id="testMicBtn" type="button">Test mic</button>
    </div>
  ...
  <script>
    const DEMO_BUILD = "pcm16-worklet-default-rate-2026-05-18a";
    ...
    stopBtn.addEventListener("click", () => stopRecording().catch((err) => log(err.message)));
    refreshBtn.addEventListener("click", () => refreshDevices(true).catch((err) => log(err.message)));
    testMicBtn.addEventListener("click", () => testMicrophone().catch((err) => log(err.message)));

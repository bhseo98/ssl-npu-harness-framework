# Easy Guide 8 — amdsharktank 커널 우회 (RoPE concat · KV gather · q8 matmul · flash attention)


> 1. `RoPEKernels.rope_select_concat` 2. `KVCacheGatherKernel` 3. `mmt_block_scaled_q8` 4. `kernels/attention.py` flash_attention.

---

## 0. 한 줄 결론

> **네 우회 모두 동기가 하나로 수렴: fusion / 중간 텐서 materialize 회피.** 표준 `torch→aten→linalg` 경로는 fusion 비친화적인 IR(concat 차단벽 · 별도 dequant weight · O(n²) score matrix · generic gather)을 만든다. amdsharktank는 같은 수학을 **IREE codegen이 fuse·tile하기 좋은 형태**로 직접 박음. 대가 = `iree_linalg_ext.*` 같은 **IREE 전용 dialect** → 비-IREE NPU엔 portability 빚이 될거라 생각됨.
...
## 1. common method - how splice?

`@mlir_kernel`(`kernels/mlir_kernel.py`) / `CustomOp.register`(`.mlir` + `inline_template_function`)는 **둘 다 Torch custom op으로 등록**된다. 그래서:

1. `torch.export`가 그 내부를 `aten.*`으로 **추적하지 않음** → 단일 opaque op.
2. `aot.export` 중 op의 `generate()`(mlir_kernel.py:295)가 Jinja2로 템플릿을 **static dim·dtype에 specialize**(dynamic dim은 안 함).
3. `iree.turbine...Merger`로 `util.func private @<name>` 을 메인 모듈에 splice, 호출부를 `util.call @<name>` 로 치환.

→ **우회 method = 표준 lowering을 건너뛰고 목표 linalg/IREE MLIR을 사람이 직접 써넣는 것.** 표준 경로가 만들 IR보다 *fusion 친화적*인 형태를 손으로 박는 게 목적.

---

## 2. AMD kernel — summurize method
...
| ④ **`flash_attention`/`masked_`** (`@mlir_kernel` ×2) | softmax-attn naive = **O(n²) score matrix materialize** + 다중 패스 | flash(online-softmax)를 **`iree_linalg_ext.attention`** 한 op으로 (score matrix 안 만듦) | `iree_linalg_ext.attention` |
...
**소스/클래스 코드 참조** (rotary_embedding_hf.py):
```python
...
- **왜**: PagedAttention 캐시는 물리적으로 흩어진 페이지를 **런타임 텐서 `page_ids`로 간접 참조**(data-dependent gather). 표준 torch `index_select`/`index` 체인은 6D paged slab의 "블록·파티션 슬라이스 후 페이지 gather" 구조에 안 맞고, IREE가 indirect DMA로 타일·스케줄하기 어렵다.
...
**source** — `@mlir_kernel` 인라인이 아니라 `CustomOp` + 외부 템플릿 변종:
```python
...
## 5. INT8 — `mmt_block_scaled_q8` 표준 재표현 스케치

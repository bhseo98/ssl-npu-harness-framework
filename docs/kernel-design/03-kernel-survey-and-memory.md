...
> 근거 산출물: `logs/whisper-int8/whisper-tiny-int8.mlir`(WM2/WM3), `src/torch_mlir_zoo/{ops,models,kernels}/`, amdsharktank 커널 우회 분석([../easy-series/easy-all.md#sec-easy8](../easy-series/easy-all.md#sec-easy8)).
...
- **footprint 범인 2개**: f32 임베딩 테이블 **80MB**(103MB vmfb의 대부분; proj_out tie 수정 후 — [04-int8-footprint](04-int8-footprint.md)) + attention **O(T²)** 중간텐서.
...
|---|---|---|---|---|---|
| **임베딩 테이블** | `embedding` 1, `51865×384xf32` 잔존 | `nn.Embedding` f32 그대로 | **79.6MB f32 = vmfb 103MB의 대부분** (llama면 ~1GB) | **INT8 block-scaled 임베딩 + gather-dequant** → ~4× 축소 | 🔴 |
...
- **weight-tying 깨짐 — ✅ 수정됨([04](04-int8-footprint.md))** — proj_out↔embed_tokens 공유가 proj_out 양자화로 분리돼 vocab 표가 이중 저장(embed f32 80MB + proj_out i8 22MB)됐었음. `quantize_linears_`가 tied Linear를 양자화에서 제외하도록 수정 → vmfb 125→103MB, rel 0.8→0.61%.
...
- **vmfb 103MB** — weight가 상수로 박혀 큰 아티팩트(§3 mmap/양자화로 해결).
...
| 기법 | 무엇 | 기대효과(우리 수치) | 사례 |
|---|---|---|---|
| **weight 외부화 + mmap** | 가중치를 vmfb에 굽지 말고 `flow.parameter.named` 외부 파일로 → mmap, OS demand-paging | vmfb 103MB→**코드만(~수MB)**, peak RSS=작업셋만, 프로세스 공유, 즉시 기동 | llama.cpp(GGUF mmap) |
| **임베딩 INT8** (re-tie는 [04](04-int8-footprint.md)에서 완료) | `nn.Embedding`을 block-scaled INT8로 (proj_out 중복은 tie 수정으로 이미 제거) | 임베딩 80→**20MB** → vmfb 103→~**46MB** | 모든 LLM |
...
3. 임베딩 INT8 → -57MB (vmfb 103→~46MB; proj_out re-tie는 [04] 완료)
4. INT4 혼합정밀     → matmul weight 추가 ~2×, 정확도 검증 게이트
5. flash-attention   → activation O(T²)→O(T), 긴 오디오
6. (생성형/llama) paged + 양자화 KV → 4× 컨텍스트
```

정적(2·3·4)과 동적(5·6)은 독립이라 곱해진다 — 임베딩 INT8 + mmap + flash-attn → vmfb ~46MB + 낮은 RSS + 긴 컨텍스트 동시.
...
| 지표 | FP32(기본) | INT8(tie 수정 전 스냅샷) | 차이 |
|---|---|---|---|
...
1. **−17%뿐이던 이유 → tie 수정으로 −32%(125→103MB)** — (수정 전) 80MB f32 임베딩이 그대로 + weight-tying 깨져 vocab 가중치 이중 저장(embed f32 80MB + proj_out i8 22MB). proj_out tie 수정([04-int8-footprint](04-int8-footprint.md))으로 중복 제거 → **vmfb 103MB, rel 0.61%**. 임베딩까지 INT8 시 추가로 **~46MB (~3×)**.
...
- 커널 우회·fusion: [../easy-series/easy-all.md#sec-easy8](../easy-series/easy-all.md#sec-easy8) · CustomOp 동작: [../easy-series/easy-all.md#sec-easy8-qna](../easy-series/easy-all.md#sec-easy8-qna)

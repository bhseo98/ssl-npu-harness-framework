...
> 본 문서는 외부 NPU compiler 측 (`agents/2026-05-20-reviewer-meeting.md`) +
> PDF `internal-spec` + 사용자 의 2026-05-26 지시를
...
> `agents/` 9 spec + `REPORT-2026-05-{20,26}-*.md` 가 단계별 deliverable 의
...
|---|---|---|---|
| 1 | Naive Application Code (Python + PyTorch) on GPU Server | `voice-app` | ✅ 완료 (`docs/reports/2026-05-19-results.md`) |
| 2 | **PyTorch Model → MLIR FRAMEWORK for target application** | **`torch-mlir-zoo` (현재)** | ⚠ 진행중 — Gap G (iree.turbine backend) 완료. real Llama 후속. |
| 3 | CPU-based Target Application on SoC Model (VP) | (미정) | ✗ 미진입 — `agents/vp.md` spec 만 |

### 1.2 PDF `internal-spec` 6 단계 매핑
...
| Step 3 (SHARK-AI 분석) | `docs/SHARK_AI_ANALYSIS.md` §2-4 + `REPORT-2026-05-26` §4.2 | ✅ |
...
graph TB
    ...
    subgraph "Compiler Stack (PDF 다이어그램)"
        ...
        SSL[("target NPU Library<br/>+ Runtime")]
        HW[("target NPU 칩 / VP")]
    end
...
→ 모든 application branch 가 framework core 한 줄도 안 바꾸고 plugin 만 추가. voice-app 만 historical 예외 (이미 Gap B 로 호환, `agents/roadmap.md`).
...
→ IR 수준 정량 검증: toy 3-block 비교 시 amdsharktank path **4859 lines** vs 자체 path **810 lines** (2-block) + `paged_attention_kv_cache_gather` 7회 출현 vs 0회. `REPORT-2026-05-26-amd-shark-integration.md` §4-5.
...
graph TD
    ...
    T -.future.-> S["(미정)<br/>SoC VP integration<br/>(Phase 3, agents/vp.md)"]
...
세 application branch 모두 framework core 한 줄도 안 바꿈 (`git diff main <branch> -- src/npu_harness_framework/` empty). voice-app 의 Gap B 가 마지막 historical 예외를 해소했음 (`agents/roadmap.md` 참고).
...
| 논의 § | 핵심 인용 | 본 repo 위치 |
|---|---|---|
| §1 시스템 목적 | *"지능을 갖고 대화 ... 임베디드 시스템"* | `agents/mission.md` + voice-app demo |
...
| §4 3번 (VP / llama.cpp + STT/TTS) | *"CPU만 ... llama.cpp ... stt랑 tts까지"* | `agents/vp.md` (미진입) |
...
| TTS lowering 후보 결정 (Piper / VITS-lite / ESPnet 유지) | Low | — | `agents/open-questions.md` Q2 |
| SoC VP 통합 (`agents/vp.md`) | Future | Phase 3 | 프로젝트 리드 VP 모델 + Q3 답변 |
...
- `agents/2026-05-20-reviewer-meeting.md` — 설계 노트 원문 (source-of-truth)
- `agents/{mission,constraints,deliverables,framework,design,model-zoo,roadmap,open-questions,vp}.md` — 9 sub-agent spec
- `docs/SHARK_AI_ANALYSIS.md` — AMD-SHARK-AI 의 server-side LLM 분석 + 본 zoo 의 매핑 + 2026-05-26 setup 절차
- `REPORT-2026-05-20-phase2-mlir.md` — Phase 2 1차 milestone (자체 path)
- `REPORT-2026-05-26-amd-shark-integration.md` — Gap G (iree.turbine backend) 정량 비교 보고
- `internal-spec` — 프로젝트 리드 PDF (6 단계 spec)

...
> **명명 의도** — "Harness"는 `lm-evaluation-harness` · HELM · `inspect-evals` 계열의
> AI evaluation harness 패턴 (동일 contract 하에 모델 swap, controlled 환경에서
> metric 측정, target 배포 전 검증) 을 의식적으로 차용한 것이다. 일반 AI harness 가
> *evaluation only* 인 반면, 본 프로젝트는 그 위에 **lowering preparation** 책임
> (locked 구성을 MLIR 로 추출하는 단계) 을 한 겹 더 얹는다 — 그래서
> "Harness **Framework**" 다.
...
GitHub `bhseo98/npu-harness-framework` 로 push 된다. 실행은 별도 GPU 머신에서
...
flowchart TB
  ...
  subgraph GITHUB["☁️ GitHub (private)"]
    Repo["bhseo98/npu-harness-framework<br/>main branch"]
  end
...
**Gap B (설계 검토 §5 "하나로 묶이게")** 이후: 최상위 `BaseStage` marker
ABC (modality-agnostic, `__call__(payload)` 하나만 강제) 가 있고, 도메인 ABC
`BaseSTT`/`BaseLLM`/`BaseTTS` 가 이를 상속한다. 도메인 ABC 는 각자 도메인
메서드 (transcribe/chat/synthesize) 를 추상으로, `__call__` 은 concrete dispatch
로 둔다. 따라서 voice stage instance 가 `isinstance(BaseStage)` 자동 충족 →
main / vision-app / torch-mlir-zoo 의 generic `Pipeline` 위에서도 plug-in 가능.
batch 처리, streaming, async 는 의도적 confine.

```mermaid
classDiagram
    class BaseStage {
        <<abstract>>
        +__call__(payload Any) Any
    }
    class BaseSTT {
        <<abstract>>
    ...
    class BaseLLM {
        ...
        +__call__(payload) str
    }
    class BaseTTS {
        <<abstract>>
        +synthesize(text) AudioBuffer
        +__call__(text) AudioBuffer
    }
    class AudioBuffer {
        <<TypedDict>>
        +samples ndarray
...
> *참고: Phase 1 의 STT/LLM/TTS 추상은 voice application 의 stage 추상이며, Phase 2 는 그보다 한 계층 위 — domain-agnostic compiler framework — 로 올라간다. Voice 는 Phase 1 의 first application 사례. 자세히는 [docs/reports/2026-05-19-results.md §5.2](reports/2026-05-19-results.md) 참조.*

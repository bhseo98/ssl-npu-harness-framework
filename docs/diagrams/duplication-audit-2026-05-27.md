...
flowchart TD
    ...
    R --> H["⑧ development.md vs<br/>(내부 문서) vs<br/>(내부 문서)]:::weak
...
gantt
    ...
    section Phase 2
    (내부 문서)     :done, 2026-05-20, 1d
    (내부 문서)  :done, 2026-05-26, 1d
    (내부 문서)  :done, 2026-05-27, 1d
```

| 파일 | 주제 | 시점 |
|---|---|:---:|
| (내부 문서) | Phase 2 (MLIR Lowering) 1차 milestone — torch-mlir-zoo 초기 (단위 op + tiny Llama) | 05-20 |
| (내부 문서) | amd-shark-tank `iree.turbine` 백엔드를 additive plug-in 으로 통합 (Gap G) | 05-26 |
| (내부 문서) | Step 5 완료 — real Llama-3.2-1B + `.vmfb` | 05-27 |
...
→ 각 commit 이 다른 surface area. (내부 문서) *"Surgical Changes"* 원칙 부합.

---

## 9. ⑧ ⚠ **약한 overlap 1곳** — development.md ↔ (내부 문서) ↔ (내부 문서)
...
| **(내부 문서)** | 81 | Gap A/B/C/D/E/F/G/G' 매트릭스 + 작업 순서 + 보류 항목 | 다음 마일스톤 |
| **(내부 문서)** | 50 | Role + 3 Responsibilities + Deliverable 부합도 정량 | Deliverable 1/2/3 표 |
...
flowchart LR
    ...
    R["(내부 문서)<br/>(작업 우선순위 관점)"]:::roadmap
    DL["(내부 문서)<br/>(요구사항 관점)"]:::deliv
...
| **B** | development.md 의 Deliverable 1/2/3 표 → (내부 문서) 로 link 만 (본문 인용 → "see deliverables.md") | 5 분 | 단일 진리원 확립. 단점: development.md 가 self-contained 아님 |
| **C** | (내부 문서) 의 "다음 마일스톤" 섹션 → development.md §6 로 흡수, (내부 문서) 는 "Gap 매트릭스" 전용으로 슬림화 | 10 분 | development.md 가 *유일한* status board, (내부 문서)* 는 *순수 sub-agent spec* |
...
| ⚠ | development.md ↔ (내부 문서) ↔ (내부 문서) | 사용자 결정 — 옵션 (A) 그대로 / (B) link only / (C) 흡수 |

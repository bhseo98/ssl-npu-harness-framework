...
> 로 같은 코드 붙여넣기 가능. 별도 `.drawio` XML 은 [`llama-step5.drawio`](diagrams/llama-step5.drawio).
...
flowchart TD
    ...
    subgraph NEXT ["다음 turn — IREE Runtime / target NPU stack (외부 (compiler stack) 측)"]
        ...
        SSL_LIB["target NPU Library"]:::next
        SSL_RT["target NPU Runtime"]:::next
        SSL_DRV["target NPU Driver"]:::next
        SSL_HW["target NPU HW"]:::next
        IREE_C --> IREE_R
        IREE_R --> IREE_HAL
        IREE_R --> IREE_VM
        IREE_HAL --> SSL_LIB --> SSL_RT --> SSL_DRV --> SSL_HW
    end
...
flowchart LR
    ...
    D["<b>Step D</b><br/>REPORT-2026-05-27 + roadmap Gap G'<br/>~ 5 min"]:::ok
...
flowchart TD
    ...
    F3["agents/roadmap.md<br/>(Gap G' 섹션 · +9 LoC)"]:::mod
    F4["REPORT-2026-05-27-llama-step5-complete.md<br/>(신규 · 정량 보고서)"]:::new

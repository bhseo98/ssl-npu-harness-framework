...
| Q-A3 | VSCode Remote-SSH에 [runtime-host] 추가 | 맥북 `~/.ssh/config` 블록 제공(§2) | 본 문서 §2 | ✅ 제공 |
...
## 2. Q-A3 — VSCode Remote-SSH에 [runtime-host] 추가

VSCode Remote-SSH config는 **맥북** `~/.ssh/config`에 있어 server5에서 직접 못 고친다. 맥북에 추가:

```sshconfig
Host [runtime-host]
    HostName <[runtime-host] tailscale IP>    # tailscale status, 또는 local memory reference_vp_server4
    User bohyun
    Port 22
    IdentityFile ~/.ssh/id_ed25519
```

- MagicDNS(`[runtime-host].<tailnet>.ts.net`)도 가능하나 IP 권장.
- **전제**: 맥북 공개키가 `[runtime-host]:~/.ssh/authorized_keys`에 등록돼야 함(`ssh-copy-id [runtime-host]`). 현재 키 등록은 [build-host] 것.
